function calculatePolarizationCurve
    clc;
    clear all;

    allX2 = 5.0:0.1:45.0;
    allX1 = 0.01:0.1:5.2;    
    [bigX,bigY] = meshgrid(allX1,allX2);
    dG_cathodic = calculateParameter(bigX,bigY,5);

    figure(100)
    hold on
    surf(bigX,bigY,dG_cathodic, 'edgecolor', 'none')
    xlabel('[Cl^-] (M)')
    ylabel('T (^oC)')
    zlabel('\Delta G_{cathodic}^{ORR} (J/mol)')
    axis square
    box on
    hold off
    ...
    ddir = fullfile('C:','Users',userName{computer},'OneDrive','Atmospheric Corrosion','PolarizationCurveAnalysis','PolarizationCurves','PolarizationData-138');
    dfn = '13-8_600MM_25C_TAPE_02_pd.csv';%'13-8_2700MM_25C_TAPE_02_pd.csv'; %'13-8_100MM_10C_TAPE_01_pd.csv'; %

    ffn = fullfile(ddir,dfn);
    if isfile(ffn)
        TT = readtable(ffn);
        rawDataPot = TT.Vf;
        absCurr = abs(TT.Im);
        dataCurr = absCurr./0.495;
        [minCurr, idxMinCurr] = min(dataCurr);

        vADJ = abs(TT.Im).*12.72;
        dataPot = zeros(size(rawDataPot));
        
        for i = 1:length(rawDataPot)
            if i <= idxMinCurr
                if rawDataPot(i) >= 0
                    dataPot(i) = rawDataPot(i) - vADJ(i); 
                else
                    dataPot(i) = rawDataPot(i) + vADJ(i);
                end
            else
...
%         figure(4)
%         hold on
%         plot(dataCurr,rawDataPot,'-r')
%         plot(dataCurr,dataPot,'-b')
%         ax = gca;
%         ax.XScale = 'log';
%         hold off
    end
...
%========================================================================== 
    ...
    figure(1)
    hold on
    plot(abs(iVals(:,end)), eApp, '-b','LineWidth',plot_line_width)
    plot(dataCurr,dataPot,':r','LineWidth',plot_line_width)
    ...
    sumVal = 0.0;
    for i = 1:nT
        eK = (dataCurr(i) - abs(iVals(i,end)))/abs(iVals(i,end));
        sumVal = sumVal + eK^2;
    end
    MSE = sqrt(sumVal)/(nT - 11);
